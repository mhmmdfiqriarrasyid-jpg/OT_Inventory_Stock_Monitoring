...
export default function TransactionIn() {
  ...
  useEffect(() => {
    const q = query(collection(db, 'items'), where('status', '==', 'in_use'))
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'items', selected.id), {
        status:    'available',
        quantity:  (selected.quantity ?? 0) + Number(form.quantity),
    ...
    <div className="max-w-2xl space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
          <ArrowDownCircle size={24} className="text-green-500" /> Item In
        </h1>
        <p className="text-sm text-gray-500">Log an item returning to inventory</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="card p-6 space-y-5">
        <div>
          <label className="label">Select Item (in-use only)</label>
          ...
          {search && !selected && (
            <div className="border border-gray-200 rounded-lg divide-y divide-gray-100 max-h-48 overflow-y-auto">
              {filtered.length === 0
                ? <p className="px-4 py-3 text-sm text-gray-400">No in-use items found</p>
                : filtered.map(item => (
                  <button
                    type="button"
                    key={item.id}
          ...
          {selected && (
            <div className="mt-1 px-3 py-2 bg-green-50 rounded-lg text-sm text-green-700 font-medium">
        ...
        <div>
          <label className="label">Reference Person (who is returning)</label>
          <input
            className="input"
            placeholder="Name of person returning item"
